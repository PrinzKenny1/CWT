...
export class RS384 implements Algorithm {
  ...
  sign(message: Buffer): Buffer {
    return crypto.sign("RSA-SHA384", message, {
      key: this.privateKey,
  ...
  verify(message: Buffer, signature: Buffer): boolean {
    return crypto.verify("RSA-SHA384", message, this.publicKey, signature);
  }

  getAlgorithm() {
    return CWTAlgorithms.RS384;
  }
