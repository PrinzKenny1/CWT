...
export class ES512 implements Algorithm {
  ...
  sign(message: Buffer): Buffer {
    return crypto.sign("SHA512", message, {
      key: this.privateKey,
  ...
  verify(message: Buffer, signature: Buffer): boolean {
    return crypto.verify("SHA512", message, this.publicKey, signature);
  }

  getAlgorithm() {
    return CWTAlgorithms.ES512;
  }
