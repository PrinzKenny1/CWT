...
export class RS256 implements Algorithm {
  ...
  sign(message: Buffer): Buffer {
    return crypto.sign("RSA-SHA256", message, {
      key: this.privateKey,
  ...
  verify(message: Buffer, signature: Buffer): boolean {
    return crypto.verify("RSA-SHA256", message, this.publicKey, signature);
  }

  getAlgorithm() {
    return CWTAlgorithms.RS256;
  }
